    ...
    if(can_issue==false){
        ...
        for(int i=0;i<=json->book_no();i++){
            if(issue_book_ID.toLower()==book_json.at(i).toObject()["id"].toString().toLower()){
                ...
                QFile book_file("C:/Users/user/Documents/LibraryMS/JSON/book_data.json");
                ...
                if(!book_file.write(doc.toJson())){
                    QMessageBox::critical(nullptr,"Error.", "Book Database couldn't be accessed.");
                    success=false;
                }
                book_file.close();
//                qDebug()<<book_json;
        ...
        if(success){
            for(int i=0;i<=json->student_no();i++){
                if(issue_stu_ID.toLower()==stu_json.at(i).toObject()["id"].toString().toLower()){
                    ...
                    QFile stu_file("C:/Users/user/Documents/LibraryMS/JSON/student_data.json");
...
void admin_page::on_pushButton_addConfirm_clicked()
{
    ...
    //    qDebug()<<s_json;
        QFile s_file("C:/Users/user/Documents/LibraryMS/JSON/student_data.json");
...
void admin_page::on_pushButton_addnewbook_clicked()
{
    ...
    //    qDebug()<<s_json;
        QFile b_file("C:/Users/user/Documents/LibraryMS/JSON/book_data.json");
...
//        qDebug()<<book_json;
        QFile s_file("C:/Users/user/Documents/LibraryMS/JSON/book_data.json");
        if (!s_file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QMessageBox::critical(nullptr,"Error", "Error in parsing book data");
        }
        QJsonDocument doc(book_json);
